  ...
  return (
    <DashboardShell navItems={learnerNav} roleLabel="Learner" roleColor="bg-gradient-to-r from-accent-500 to-accent-600">
      <div className="mb-6">
        ...
        <p className="text-sm text-slate-500">Chat with your teachers</p>
      </div>
              ...
              conversations.map((conv) => (
                <button key={conv.partnerId} onClick={() => openConversation(conv.partnerId)}
                  className={`w-full p-4 text-left hover:bg-slate-50 transition-colors ${
                    selectedPartner === conv.partnerId ? "bg-accent-50" : ""
                  }`}>
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-accent-100 flex items-center justify-center text-accent-600 font-bold text-sm">
                      {conv.partnerName[0]}
                    </div>
            ...
            <>
                    ...
                    return (
                      <div key={msg.id} className={`flex ${isMine ? "justify-end" : "justify-start"}`}>
                        <div className={`max-w-[75%] p-3 rounded-2xl ${
                          isMine ? "bg-accent-500 text-white rounded-br-sm" : "bg-slate-100 text-slate-700 rounded-bl-sm"
                        }`}>
                          <p className="text-sm">{msg.content}</p>
                          <p className={`text-[10px] mt-1 ${isMine ? "text-accent-100" : "text-slate-400"}`}>
              ...
              <form onSubmit={sendMessage} className="p-3 border-t border-slate-100 flex gap-2">
                ...
                <button type="submit" disabled={sending || !newMessage.trim()}
                  className="px-6 py-3 rounded-xl bg-gradient-to-r from-accent-500 to-accent-600 text-white font-semibold disabled:opacity-50">
                  Send
                </button>
              </form>
            </>
